...
import waitForResult from 'src/helpers/waitForResult';
...
registerSuite(function(this: Test) {
	...
	return {
		name: 'leadfoot/helpers/waitForResult',
		...
		'basic test'() {
			return command
				...
				.click()
				.then(
					waitForResult('return document.getElementById("d");', [], 1000)
				)
		...
		'without args'() {
			return command
				...
				.findById('makeD')
				.click()
				.then(waitForResult('return document.getElementById("d");', 1000))
				.then(function(result: any) {
					assert.property(
						result,
						'elementId',

...
export default class Session extends Searchable< Task<Element>, Task<Element[]>, Task<void>> {
	 ...
	 * @private
	 */
	private _delegateToServer<T>(method: string, path: string, requestData: any, pathParts?: string[]) {
		...
		return new Task<T>((resolve, reject) => {
			...
			const runRequest = () => {
				...
				const response = (<any>this._server)[method](path, requestData, pathParts).then(returnValue);
	...
	private _get<T>(path: string, requestData?: any, pathParts?: string[]) {
		return this._delegateToServer<T>('_get', path, requestData, pathParts);
	}

	private _post<T>(path: string, requestData?: any, pathParts?: string[]) {
		return this._delegateToServer<T>('_post', path, requestData, pathParts);
	}

	private _delete<T>(path: string, requestData?: any, pathParts?: string[]) {
		return this._delegateToServer<T>('_delete', path, requestData, pathParts);
	}
	...
	setTimeout(type: string, ms: number) {
		...
		const promise = this._post<void>('timeouts', {
			type: type,
			ms: ms
		}).catch(error => {
			...
			if (error.name === 'UnknownCommand') {
				if (type === 'script') {
					return this._post<void>('timeouts/async_script', { ms: ms });
				}
				else if (type === 'implicit') {
					return this._post<void>('timeouts/implicit_wait', { ms: ms });
				}
			}
	...
	getCurrentWindowHandle() {
		return this._get<string>('window_handle').then(handle => {
	 ...
	 */
	getAllWindowHandles() {
		return this._get<string[]>('window_handles').then((handles: string[]) => {
	...
	get(url: string) {
		...
		return this._post<void>('url', { url: url });
	}
	...
	goForward() {
		...
		return this._post<void>('forward');
	}
	...
	refresh() {
		...
		return this._post<void>('refresh');
	}
	...
	execute<T>(script: Function | string, ...args: any[]): Task<T> {
		...
		let result = this._post<T>('execute', {
	...
	executeAsync<T>(script: Function | string, args?: any[]) {
		...
		return this._post<T>('execute_async', {
			script: util.toExecuteString(script),
			args: args || []
	 ...
	 */
	takeScreenshot() {
		return this._get<string>('screenshot').then(data => new Buffer(data, 'base64'));
	}
	...
	getAvailableImeEngines() {
		return this._get<string[]>('ime/available_engines');
	}
	 ...
	 */
	getActiveImeEngine() {
		return this._get<string>('ime/active_engine');
	}
	 ...
	 */
	isImeActivated() {
		return this._get<boolean>('ime/activated');
	}
	 ...
	 */
	deactivateIme() {
		return this._post<void>('ime/deactivate');
	}
	...
	activateIme(engine: string) {
		return this._post<void>('ime/activate', { engine: engine });
	}
	...
	switchToFrame(id: string | number | Element) {
		return this._post<void>('frame', { id: id });
	}
	 ...
	 */
	switchToParentFrame() {
		return this._post<void>('frame/parent').catch(error => {
	...
	closeCurrentWindow() {
		...
		return this._delete<void>('window').catch(error => {
	...
	setWindowSize(...args: any[]) {
		...
		if (this.capabilities.implicitWindowHandles) {
			if (windowHandle == null) {
				return this._post<void>('window/size', data);
			}
			else {
			...
			return this._post<void>('window/$0/size', {
				width: width,
				height: height
			}, [windowHandle]);
		}
	...
	getWindowSize(windowHandle?: string) {
		if (this.capabilities.implicitWindowHandles) {
			if (windowHandle == null) {
				return this._get<{ width: number, height: number }>('window/size');
			}
			else {
				// User provided a window handle; get the current handle, switch to the new one, get the size, then
				// switch back to the original handle.
			...
			return this._get<{ width: number, height: number }>('window/$0/size', null, [windowHandle]);
		}
	}
	...
	setWindowPosition(...args: any[]) {
		...
		return this._post<void>('window/$0/position', {
			x: x,
			y: y
		}, [windowHandle]);
	}
	...
	getWindowPosition(windowHandle?: string) {
		...
		return this._get<{ x: number, y: number }>('window/$0/position', null, [windowHandle]).then(function (position) {
	...
	maximizeWindow(windowHandle?: string) {
		...
		return this._post<void>('window/$0/maximize', null, [windowHandle]);
	}
	 ...
	 */
	getCookies() {
		return this._get<WebDriverCookie[]>('cookie').then(function (cookies: WebDriverCookie[]) {
	...
	clearCookies() {
		...
		return this._delete<void>('cookie');
	}
	...
	deleteCookie(name: string) {
		...
		return this._delete<void>('cookie/$0', null, [name]);
	}
	...
	getPageSource() {
		if (this.capabilities.brokenPageSource) {
			...
			return this._get<string>('source');
		}
	}
	...
	findAll(using: string, value: string) {
		...
		return this._post<any[]>('elements', {
			using: using,
			value: value
		}).then(elements => {
			return elements.map((element: ElementOrElementId) => {
	...
	getActiveElement() {
		...
		if (this.capabilities.brokenActiveElement) {
			...
			return this._post<any>('element/active').then((element: ElementOrElementId) => {
				if (element) {
					return new Element(element, this);
				}
	 ...
	 */
	getOrientation() {
		return this._get<'portrait' | 'landscape'>('orientation').then(function (orientation) {
	...
	setOrientation(orientation: string) {
		...
		return this._post<void>('orientation', {
	 ...
	 */
	getAlertText() {
		return this._get<string>('alert_text');
	}
	...
	typeInPrompt(text: string | string[]) {
		...
		return this._post<void>('alert_text', {
			text: text
		});
	}
	 ...
	 */
	acceptAlert() {
		return this._post<void>('accept_alert');
	}
	 ...
	 */
	dismissAlert() {
		return this._post<void>('dismiss_alert');
	}
	...
	pressMouseButton(button?: number) {
		...
		return this._post<void>('buttondown', {
			button: button
		});
	}
	...
	releaseMouseButton(button?: number) {
		...
		return this._post<void>('buttonup', {
			button: button
		});
	}
	...
	doubleClick() {
		...
		if (this.capabilities.brokenDoubleClick) {
			...
			}).then(() => this._post<void>('doubleclick'));
		}

		return this._post<void>('doubleclick');
	}
	...
	@forCommand({ usesElement: true })
	tap(element: Element) {
		return this._post<void>('touch/click', { element: element.elementId });
	}
	...
	pressFinger(x: number, y: number) {
		...
		return this._post<void>('touch/down', { x: x, y: y });
	}
	...
	releaseFinger(x: number, y: number) {
		return this._post<void>('touch/up', { x: x, y: y });
	}
	...
	moveFinger(x: number, y: number) {
		return this._post<void>('touch/move', { x: x, y: y });
	}
	...
	touchScroll(...args: any[]) {
		...
		return this._post<void>('touch/scroll', {
			element: element,
			xoffset: xOffset,
			yoffset: yOffset
		});
	...
	doubleTap(element?: Element) {
		...
		return this._post<void>('touch/doubleclick', { element: elementId });
	}
	...
	longTap(element?: Element) {
		const elementId = element && element.elementId;
		return this._post<void>('touch/longclick', { element: elementId });
	}
	...
	flickFinger(...args: any[]) {
		...
		if (typeof speed === 'undefined' && typeof yOffset === 'undefined' && typeof xOffset !== 'undefined') {
			return this._post<void>('touch/flick', {
				xspeed: element,
				yspeed: xOffset
			});
		}
	 ...
	 */
	getGeolocation() {
		return this._get<Geolocation>('location').then(location => {
	...
	setGeolocation(location: Geolocation) {
		...
		return this._post<void>('location', { location });
	}
	...
	getLogsFor(type: string) {
		return this._post<string[] | LogEntry[]>('log', {
			type: type
		}).then(function (logs) {
	...
	getAvailableLogTypes() {
		...
		return this._get<string[]>('log/types');
	}
	...
	getApplicationCacheStatus() {
		return this._get<number>('application_cache/status');
	}
	 ...
	 */
	getLocalStorageKeys() {
		return this._get<string[]>('local_storage');
	}
	...
	setLocalStorageItem(key: string, value: string) {
		return this._post<void>('local_storage', { key, value });
	}
	 ...
	 */
	clearLocalStorage() {
		return this._delete<void>('local_storage');
	}
	...
	getLocalStorageItem(key: string) {
		return this._get<string>('local_storage/key/$0', null, [key]);
	}
	...
	deleteLocalStorageItem(key: string) {
		return this._delete<void>('local_storage/key/$0', null, [key]);
	}
	...
	getLocalStorageLength() {
		return this._get<number>('local_storage/size');
	}
	...
	getSessionStorageKeys() {
		return this._get<string[]>('session_storage');
	}
	...
	setSessionStorageItem(key: string, value: string) {
		return this._post<void>('session_storage', { key, value });
	}
	 ...
	 */
	clearSessionStorage() {
		return this._delete<void>('session_storage');
	}
	...
	getSessionStorageItem(key: string) {
		return this._get<string>('session_storage/key/$0', null, [key]);
	}
	...
	deleteSessionStorageItem(key: string) {
		return this._delete<void>('session_storage/key/$0', null, [key]);
	}
	...
	getSessionStorageLength() {
		return this._get<number>('session_storage/size');
	}
